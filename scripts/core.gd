...
var login = ""
var state: State.Core = State.Core.INIT
...
func leave():
	if state == State.Core.LEAVING || state == State.Core.STOPPING_GAME:
		return
	assert(state != State.Core.QUITTING && state != State.Core.LOADING)
	print("Leaving...")
